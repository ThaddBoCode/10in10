...
import { BarChart2, Scale, Flame, Activity, Heart, Camera, Upload, Plus } from "lucide-react";
...
export default function ChartsPage() {
  ...
  useEffect(() => {
    fetch("/api/goals").then((r) => r.json()).then((d) => setGoal(d.goal));
  ...
  const handlePhotoUpload = async (file: File) => {
    ...
    formData.append("weekNumber", String(uploadWeek));
            ...
            {Array.from({ length: goal?.weeks || 10 }, (_, i) => {
              ...
              return (
                <button
                  ...
                  style={{
                    background: photo ? "var(--bg-card)" : "var(--bg-card)",
          ...
          {weights.length > 0 ? (
            <div className="text-center">
              <p className="font-numbers text-[48px]" style={{ color: "var(--text)" }}>
                {/* Use 180cm default if no height set */}
