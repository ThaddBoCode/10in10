...
import {
  Users, Plus, Copy, Check, Trophy, Lock, Unlock,
  X, Flame, ArrowRight, Settings, UserPlus,
} from "lucide-react";
...
export default function TeamPage() {
  ...
  const loadPrivacy = useCallback(async () => {
    ...
    const data = await res.json();
    if (data.settings) {
      setPrivacyWeight(data.settings.weightVisible);
      setPrivacyCalories(data.settings.caloriesVisible);
      setPrivacyActivities(data.settings.activitiesVisible);
    }
  }, []);
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    const res = await fetch("/api/teams", {
      ...
      body: JSON.stringify({
        name: teamName,
        targetKg: parseFloat(teamTarget),
        weeks: parseInt(teamWeeks),
      }),
    });
    if (res.ok) {
      setShowCreate(false);
  ...
  return (
    <div className="pt-6">
      ...
      {teamDetail && activeTeam && (
        <>
          <div className="mx-5 mt-4 overflow-hidden rounded-[var(--card-radius)] p-6 relative"
          ...
          <div className="mt-5 flex items-center justify-between px-5">
            ...
            <span className="font-body text-xs" style={{ color: "var(--text-muted)" }}>
              <Flame size={12} className="inline" /> {teamDetail.teamTotalCalories.toLocaleString()} kcal Team
            </span>
          </div>
            ...
            <div key={m.userId} className="card mx-5 mt-2 flex items-center gap-3.5 p-3.5">
              ...
              <div className="flex-1">
                ...
                <div className="mt-1 flex items-center gap-2">
                  ...
                  {m.activityCount !== null && (
                    <span className="font-body text-xs" style={{ color: "var(--text-secondary)" }}>
                      &bull; {m.activityCount} Trainings
                    </span>
                  )}
