...
import {
  ...
  Clock, MapPin, Timer, HeartPulse, Gauge, Plus, X,
  RefreshCw, Check,
} from "lucide-react";
...
export default function ActivityPage() {
  ...
  const loadData = async () => {
    const [actRes, stravaRes] = await Promise.all([
      fetch("/api/activities?days=30"),
      fetch("/api/strava/connect"),
    ]);
    const actData = await actRes.json();
    setActivities(actData.activities || []);
    setStravaConnected(stravaRes.ok);
  };
  ...
  return (
    <div className="pt-6">
      ...
      <div className="mx-5 mt-4 flex items-center gap-3 rounded-[var(--card-radius)] p-4"
           style={{
             background: stravaConnected
               ? "linear-gradient(135deg, #FC4C02, #FF7043)"
               : "var(--bg-card)",
        ...
        <div className="flex-1">
          ...
          <p className="font-body text-xs" style={{ color: stravaConnected ? "rgba(255,255,255,0.8)" : "var(--text-secondary)" }}>
            {stravaConnected ? "Automatischer Sync aktiv" : "Verbinde im Profil"}
          </p>
        </div>
        {stravaConnected && (
